					...
					<?php foreach ($branches as $item): ?>		
						<tr>
							...
							<div class="btn-toolbar">
								<div class="btn-group">
								<button type="button" class="btn btn-info btn-small" onclick="pickBranch('<?php echo $item->first_name; ?>', '<?php echo $item->surname; ?>','<?php echo $item->phone_number; ?>','<?php echo $item->national_id; ?>')"> Pick
									</button>
								</div>
								</div>
...
<script>
	function pickBranch(first_name,surname,phone_no,id_no)
	{
		document.getElementById('patient').value=first_name;
